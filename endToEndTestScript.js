...
class ApiClient {
  ...
  async uploadAvatar(pesId) {
    ...
    formData.append('files', getImageFromBase64(TEST_DATA.fotografiaBase64), `avatar${pesId}.png`);
...
async function main() {
  try {
    ...
    const efetivo = await api.post('/api/servidores-efetivos', {
      pesNome: TEST_DATA.efetivo.nome,
      pesDataNascimento: new Date('2000-01-01').toISOString(),
    ...
    const enderecoEfetivo = await api.post(`/api/servidores-efetivos/${efetPesId}/enderecos`, {
      endTipoLogradouro: "Rua",
      endLogradouro: "Fictício Efetivo",
      endNumero: 10,
    ...
    const temporario = await api.post('/api/servidores-temporarios', {
      pesNome: TEST_DATA.temporario.nome,
      pesDataNascimento: new Date('2008-01-01').toISOString(),
