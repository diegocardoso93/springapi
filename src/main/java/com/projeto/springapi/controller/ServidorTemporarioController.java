...
public class ServidorTemporarioController {
    ...
    @Operation(summary = "Lista todos os servidores temporários com paginação",
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "204",
                    description = "Nenhum servidor temporário encontrado", content = @Content)})
    @GetMapping
    public ResponseEntity<Page<ServidorTemporarioDTO>> getAllServidoresTemporarios(@Parameter(
        ...
        Page<ServidorTemporarioDTO> servidores =
                servidorTemporarioService.getAllServidoresTemporarios(pageable);
        return servidores.isEmpty() ? ResponseEntity.noContent().build()
                : ResponseEntity.ok(servidores);
    }

    @Operation(summary = "Busca um servidor temporário por ID",
    ...
    @ApiResponses(
            value = {
                    ...
                    @ApiResponse(responseCode = "400", description = "Dados de entrada inválidos",
                            content = @Content)})
    @PostMapping
    public ResponseEntity<ServidorTemporarioDTO> createServidorTemporario(@Parameter(
            description = "Dados do servidor temporário a serem criados", required = true,
            schema = @Schema(
                    implementation = ServidorTemporarioDTO.class)) @RequestBody ServidorTemporarioDTO servidorTemporarioDTO) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(servidorTemporarioService.createServidorTemporario(servidorTemporarioDTO));
    }

    @Operation(summary = "Atualiza um servidor temporário existente",
    ...
    public ResponseEntity<ServidorTemporarioDTO> updateServidorTemporario(
            @Parameter(description = "ID do servidor temporário a ser atualizado",
                    required = true) @PathVariable Long id,
            @Parameter(description = "Novos dados do servidor temporário", required = true,
                    schema = @Schema(
                            implementation = ServidorTemporarioDTO.class)) @RequestBody ServidorTemporarioDTO servidorTemporarioDTO) {
        return ResponseEntity
                .ok(servidorTemporarioService.updateServidorTemporario(id, servidorTemporarioDTO));
    }

    @Operation(summary = "Exclui um servidor temporário por ID",
            description = "Remove um servidor temporário específico com base no seu ID.",
            tags = {"Servidores Temporários"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204",
                    description = "Servidor temporário excluído com sucesso", content = @Content),
    ...
    public ResponseEntity<Void> deleteServidorTemporario(
            @Parameter(description = "ID do servidor temporário a ser excluído",
                    required = true) @PathVariable Long id) {
