...
public class UnidadeController {
    ...
    @GetMapping
    public ResponseEntity<Page<UnidadeDTO>> getAllUnidades(
    ...
    public ResponseEntity<UnidadeDTO> getUnidadeById(
            @Parameter(description = "ID da unidade a ser buscada", required = true) @PathVariable Long id) {
    ...
    @PostMapping
    public ResponseEntity<UnidadeDTO> createUnidade(
            @Parameter(description = "Dados da unidade a serem criados", required = true,
                    schema = @Schema(implementation = UnidadeDTO.class)) @RequestBody UnidadeDTO unidadeDTO) {
    ...
    public ResponseEntity<UnidadeDTO> updateUnidade(
            @Parameter(description = "ID da unidade a ser atualizada", required = true) @PathVariable Long id,
            @Parameter(description = "Novos dados da unidade", required = true,
                    schema = @Schema(implementation = UnidadeDTO.class)) @RequestBody UnidadeDTO unidadeDTO) {
    ...
    public ResponseEntity<Void> deleteUnidade(
            @Parameter(description = "ID da unidade a ser excluída", required = true) @PathVariable Long id) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Endereço adicionado com sucesso",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = Unidade.class))), // Assuming Unidade is the entity
            @ApiResponse(responseCode = "404", description = "Unidade não encontrada",
                    content = @Content),
    ...
    public ResponseEntity<Unidade> addEnderecoToUnidade(
            @Parameter(description = "ID da unidade para adicionar o endereço", required = true) @PathVariable Integer unidadeId,
            @Parameter(description = "Dados do endereço a serem adicionados", required = true,
                    schema = @Schema(implementation = EnderecoDTO.class)) @RequestBody EnderecoDTO enderecoDTO) {
