...
public class LotacaoController {
    ...
    public ResponseEntity<LotacaoDTO> getLotacaoById(
            @Parameter(description = "ID da lotação a ser buscada", required = true) @PathVariable Long id) {
    ...
    @Operation(summary = "Criar uma nova lotação",
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Dados da lotação a serem criados",
                    required = true,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = LotacaoDTO.class))),
            responses = {
    ...
    @Operation(summary = "Atualizar uma lotação existente",
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Novos dados para a lotação",
                    required = true,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = LotacaoDTO.class))),
            responses = {
    ...
    public ResponseEntity<Void> deleteLotacao(
            @Parameter(description = "ID da lotação a ser excluída", required = true) @PathVariable Long id) {
